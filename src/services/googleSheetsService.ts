...
export async function syncViaAppsScript(
  ...
  // Try via server proxy first to avoid any browser CORS issues
  try {
    const res = await fetch('/api/gas/sync', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ webhookUrl, payload: bodyData }),
    });
    ...
    // If backend proxy failed or in standalone preview, fallback to direct text/plain POST
    try {
...
export async function fetchViaAppsScript(
  ...
  try {
    const res = await fetch(`/api/gas/read?webhookUrl=${encodeURIComponent(webhookUrl)}&action=readAll`);
    ...
    const directRes = await fetch(`${webhookUrl}?action=readAll`, { redirect: 'follow' });
