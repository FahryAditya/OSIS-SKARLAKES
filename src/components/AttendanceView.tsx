              ...
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="bg-slate-100/75 text-slate-600 font-bold uppercase text-2xs tracking-wider border-b border-slate-200">
                  <tr>
                    ...
                    <th className="py-3.5 px-4 hidden md:table-cell">Divisi & Jabatan</th>
                        ...
                        <tr key={member.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-center">
                            <div className="inline-flex items-center p-1 bg-slate-100 rounded-xl space-x-1">
                              <button
                                title="Set Hadir"
                                onClick={() => onUpdateRecordStatus(currentEvent.id, member.id, 'hadir')}
                              ...
                              <button
                                title="Set Izin"
                                onClick={() => {
                                  const reason = prompt('Masukkan keterangan izin:');
                                  if (reason !== null) {
                                    onUpdateRecordStatus(currentEvent.id, member.id, 'izin', reason || undefined);
                                  }
                                }}
                                className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                                  status === 'izin'
                              ...
                              <button
                                title="Set Sakit"
                                onClick={() => {
                                  const reason = prompt('Masukkan keterangan sakit:');
                                  if (reason !== null) {
                                    onUpdateRecordStatus(currentEvent.id, member.id, 'sakit', reason || undefined);
                                  }
                                }}
                                className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                                  status === 'sakit'
                              ...
                              <button
                                title="Set Alpa"
                                onClick={() => onUpdateRecordStatus(currentEvent.id, member.id, 'alpa')}
                                className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${
                                  status === 'alpa'
            ...
            <button
              onClick={() => {
                const rows: (string | number)[][] = [
                  ...
                  [''],
                  ['No', 'NIM', 'Nama Anggota', 'Divisi', ...events.map(e => e.title), 'Total Hadir', 'Persentase'],
                ];
