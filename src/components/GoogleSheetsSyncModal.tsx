...
export const GoogleSheetsSyncModal: React.FC<GoogleSheetsSyncModalProps> = ({
  ...
  const handleSaveAndSyncViaAppsScript = async () => {
    const url = sanitizeAppsScriptUrl(appsScriptUrl);
    ...
    try {
      ...
      console.error(err);
      if (err.message?.includes('AUTH_REQUIRED_FOR_GAS') || err.message?.includes('Siapa saja') || err.message?.includes('Izin Akses')) {
  ...
  const handlePullViaAppsScript = async () => {
    const url = sanitizeAppsScriptUrl(appsScriptUrl);
    if (!url || !url.startsWith('http')) {
      showStatus('error', 'Masukkan URL Google Apps Script Web App (/exec) yang valid terlebih dahulu.');
      return;
    }
